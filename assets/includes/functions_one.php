...
function FL_RegisterUser($registration_data,$invited = false) {
    global $fl, $sqlConnect;
    if (empty($registration_data)) {
        return false;
    }
    if ($fl['config']['registration'] == 2 && $invited == false) {
        return false;
    }
    $ip     = '0.0.0.0';
    ...
    if ($query) {
        if ($invited) {
            @DeleteAdminInvitation('code', $invited);
        }
        return true;
    } else {
        return false;
...
function FL_UpdateUserData($user_id = 0, $update_data = array()) {
    ...
    if ($user_id != $fl['user']['user_id']) {
        ...
        if (FL_IsModerator() && FL_IsAdmin($user_id)) {
            return false;
        }
    }
    $update = array();
...
function FL_ImportImageFromLogin($media, $crop = 1) {
    global $fl;
    ...
    if (move_uploaded_file($data['file'], $filename)) {
        if ($second_file == 'jpg' || $second_file == 'jpeg' || $second_file == 'png' || $second_file == 'gif') {
            ...
            if ($type == 1) {
                ...
                @FL_Resize_Crop_Image(800, 480, $filename, $last_file, 60);
...
function FL_InsertPost($news_data = array(), $post_type = '') {
    ...
    if ($query) {
        ...
        if (!FL_IsPRO()) {
            $db->where('user_id',$fl['user']['id'])->update(T_USERS,array(
                'posts' => ($fl['user']['posts'] += 1)
            ));
        }
...
function FL_InsertEntries($id = 0, $post_id = 0, $data = array()) {
    ...
    if ($data['entry_page'] == 'trivia' || $data['entry_page'] == 'flip') {
        $entry_page        = 'quiz';
    }
    if (!empty($data['entry_page'])) {
        if (in_array($data['entry_page'], $entry_pages_array)) {
    ...
    if (!empty($data['data_inputs'])) {
        ...
        if ($data['type'] == 'text') {
            ...
            $video_url   = FL_Secure($data['data_inputs']['entry_video_url']);
            $video_src   = FL_GetMediaSource(FL_Secure($data['data_inputs']['entry_video_src']));
            if (empty($video_src) && empty($video_url) && $data['data_inputs']['entry_video_type'] != 'live') {
                return false;
            } else if (empty($video_url)) {
                $video_url = $video_src;
            }
            ...
            $price = 0;
            if (!empty($data['data_inputs']['entry_question_price']) && is_numeric($data['data_inputs']['entry_question_price']) && $data['data_inputs']['entry_question_price'] > 0) {
                $price = FL_Secure($data['data_inputs']['entry_question_price']);
            }
            $question_title = FL_Secure($data['data_inputs']['entry_question_name']);
            $entry_image    = FL_GetMediaSource($data['data_inputs']['entry_image']);
            $insert_keys    = '(`user_id`,`post_id`,`index_id`, `entry_type`,`image`,`time`,`title`,`price`)';
            $insert_value   = "('{$user_id}','{$post_id}','{$index_id}','question','{$entry_image}','{$time}','{$question_title}','{$price}')";
            $sql            = "INSERT INTO {$table} {$insert_keys} VALUES {$insert_value}";
            $query          = mysqli_query($sqlConnect, $sql);
            if ($query) {
                $options   = true;
                ...
                foreach ($data['data_inputs']['answers'] as $key => $value) {
                    ...
                    $correct = 0;
                    if (!empty($value['entry_answer_radio'])) {
                        $correct = 1;
                    }
                    $registration_data = array(
                        'entry_id' => $sql_id,
                        'result_index' => !empty($value['entry_answer_result']) ? $value['entry_answer_result'] : '',
                        'text' => $value['entry_answer_text'],
                        'image' => FL_GetMediaSource($value['entry_answer_img']),
                        'time' => $time,
                        'type' => '',
...
function FL_GetMusic($id = '', $page = 0) {
    ...
    $fetched_data['publisher']   = FL_UserData($fetched_data['user_id']);
    $fetched_data['views']       = number_format($fetched_data['views']);
    $fetched_data['cat_name']    = $fl['music_categories'][$fetched_data['category']];
    return $fetched_data;
}
function FL_GetLists($id = '', $page = 0) {
    ...
    $fetched_data['publisher']   = FL_UserData($fetched_data['user_id']);
    $fetched_data['views']       = number_format($fetched_data['views']);
    $fetched_data['cat_name']    = $fl['list_categories'][$fetched_data['category']];
    return $fetched_data;
}
function FL_GetQuizzes($id = '', $page = 0) {
    ...
    $fetched_data['publisher']   = FL_UserData($fetched_data['user_id']);
    $fetched_data['views']       = number_format($fetched_data['views']);
    $fetched_data['cat_name']    = $fl['quiz_categories'][$fetched_data['category']];
    return $fetched_data;
}
function FL_GetNews($id = '', $page = 0) {
    ...
    $fetched_data['publisher']   = FL_UserData($fetched_data['user_id']);
    $fetched_data['views']       = number_format($fetched_data['views']);
    $fetched_data['cat_name']    = $fl['news_categories'][$fetched_data['category']];
    return $fetched_data;
}
function FL_GetVideos($id = '', $page = 0) {
    ...
    $fetched_data['publisher']   = FL_UserData($fetched_data['user_id']);
    $fetched_data['views']       = number_format($fetched_data['views']);
    $fetched_data['cat_name']    = $fl['video_categories'][$fetched_data['category']];
    return $fetched_data;
}
function FL_GetPolls($id = '', $page = 0) {
    ...
    $fetched_data['publisher']   = FL_UserData($fetched_data['user_id']);
    $fetched_data['views']       = number_format($fetched_data['views']);
    $fetched_data['cat_name']    = $fl['poll_categories'][$fetched_data['category']];
    return $fetched_data;
}
...
function FL_IsModerator($user_id = 0) {
    global $fl, $sqlConnect;
    if ($fl['loggedin'] == false) {
        return false;
    }
...
function FL_DeletePost($id, $type) {
    ...
    foreach ($data_field as $key => $value) {
        if (!empty($value['image'])) {
            ...
            $delete_from_s3 = FL_DeleteFromToS3($url2);
        }
    }
    ...
    if (!empty($data_entry)) {
        foreach ($data_entry as $key => $value) {
            ...
            if (!empty($value['video_url'])) {
                if (file_exists($value['video_url'])) {
                    @unlink($value['video_url']);
                }
                $delete_from_s3 = FL_DeleteFromToS3($value['video_url'],array('live' => 1));
            }
        }
...
function FL_ForEachEntries($entries_array = array()) {
    ...
    foreach ($entries_array as $fl['key'] => $fl['entry']) {
        ...
        if ($fl['entry']['entry_type'] == 'video') {
            ...
            } else if ($fl['entry']['video_type'] == 'live') {
                if (!empty($fl['videos']['stream_name']) && !empty($fl['entry']['video_url'])) {
                    $fl['entry']['video_url'] = "https://" . $fl['config']['bucket_name_2'] . ".s3.amazonaws.com/" . substr($fl['entry']['video_url'], strpos($fl['entry']['video_url'], 'upload/'));
                }
                else{
                    $fl['entry']['video_url'] = '';
                }
                $html = FL_LoadPage('players/live', array(
...
function FL_DeleteUser($user_id = 0) {
    ...
    if ($fl['user']['user_id'] != $user_data['user_id']) {
        ...
        if (FL_IsModerator() && $user_data['admin'] == 1) {
            return false;
        }
    }
...
function FL_GetStoryComments($args = array()) {
    ...
    if (!empty($args['limit']) && is_numeric($args['limit']) && $args['limit'] > 0) {
        $limit = FL_Secure($args['limit']);
    }
    $sql   = "SELECT `id` FROM $table WHERE `news_id` = '{$post}' {$sub_sql} ORDER BY `id` DESC LIMIT ".$limit;
    $data  = array();
    $query = mysqli_query($sqlConnect, $sql);
    while ($fetched_data = mysqli_fetch_assoc($query)) {
...
function FL_GetCommReplies($comm_id = false,$limit = 0,$offset = 0) {
    global $fl, $sqlConnect;
    ...
    $sql   = "SELECT `id` FROM $table WHERE `comment` = '{$comm_id}' {$sub_sql} ORDER BY `id` DESC";
    if (!empty($limit) && is_numeric($limit) && $limit > 0) {
        $sql .= " LIMIT ".FL_Secure($limit);
    }
    $data  = array();
    $query = mysqli_query($sqlConnect, $sql);
    while ($fetched_data = mysqli_fetch_assoc($query)) {
...
function FL_DeleteReaction($post_id = false, $option_id = false) {
    global $sqlConnect, $fl;
...
function FL_DeleteField($id = false) {
    global $fl, $sqlConnect;
    if ($fl['loggedin'] == false || (!FL_IsAdmin() && !FL_IsModerator())) {
        return false;
    }
    $id    = FL_Secure($id);
    $table = T_PR_FIELDS;
...
function FL_DeleteFromToS3($filename, $config = array()) {
    global $fl;
    if ($fl['config']['amazone_s3_2'] == 1 && !empty($config) && $config['live'] == 1) {
        $s3Config = (
            empty($fl['config']['amazone_s3_key_2'])   ||
            empty($fl['config']['amazone_s3_s_key_2']) ||
            empty($fl['config']['region_2'])           ||
            empty($fl['config']['bucket_name_2']));
        ...
        $s3->deleteObject([
            'Bucket' => $fl['config']['bucket_name_2'],
            'Key'    => $filename,
        ]);
        if (!$s3->doesObjectExist($fl['config']['bucket_name_2'], $filename)) {
            return true;
        }
    }
    else{
...
function FL_GetMediaSource($url = false, $crop = 0){
    ...
    if (strpos($url, "upload") === false) {
        $url = FL_ImportImageFromLogin($url, 0);
        $url = substr($url, strpos($url, "upload"), 120);
    }
    if (!file_exists($url) && !empty($crop)) {
        $url = FL_ImportImageFromLogin($url_full, 0);
        $url = substr($url, strpos($url, "upload"), 120);
    }
    $url2_explode = explode('.', $url);
    $url2 = $url2_explode[0] . '_hd.' . $url2_explode[1];
    if (!empty($crop)) {
        $resize = FL_Resize_Crop_Image(1920, 1080, $url, $url2, 60);
        $resize = FL_Resize_Crop_Image(600, 320, $url, $url, 60);
        if ($fl['config']['amazone_s3'] == 1) {
            $upload = FL_UploadToS3($url);
            $upload = FL_UploadToS3($url2);
        }
    }
    return trim($url);
}

function FL_DeleteUserAD($id = false){
    global $fl,$db;
    ...
    if (!empty($ad) || (!FL_IsAdmin() && !FL_IsModerator())) {
        ...
        if (file_exists($ad->media_file)) {
            ...
            @FL_DeleteFromToS3($ad->media_file);
        }
    }
    
    return $del;
}
function FL_IsCorrectAnswer($answer_id = false) {
    global $fl, $sqlConnect;
    if (empty($answer_id) || !is_numeric($answer_id) || $answer_id < 1) {
        return false;
    }
    $data        = array();
    $answer_id = FL_Secure($answer_id);
    $table       = T_ANSWERS;
    $query_one   = "SELECT COUNT(id) as count FROM `$table` WHERE `id` = {$answer_id} AND `correct` = 1";
    $sql       = mysqli_query($sqlConnect, $query_one);
    $sql_fetch = mysqli_fetch_assoc($sql);
    if ($sql_fetch['count'] > 0) {
        return true;
...
function FL_GetAnswerById($id = false) {
    global $fl, $sqlConnect;
    if (empty($id) || !is_numeric($id)) {
        return false;
    }
...
function FL_GetEntryById($id = false) {
    global $fl, $sqlConnect;
    if (empty($id) || !is_numeric($id)) {
        return false;
    }
...
function FL_GetQuestionsCount($post_id = false) {
    global $fl, $sqlConnect;
    ...
    $post_id = FL_Secure($post_id);
    $table       = T_ENTRIES;
    $query_one   = "SELECT COUNT(id) as count FROM `$table` WHERE `post_id` = {$post_id} AND `entry_type` = 'question'";
    $sql       = mysqli_query($sqlConnect, $query_one);
    $sql_fetch = mysqli_fetch_assoc($sql);
    return $sql_fetch['count'];
}
function FL_GetAllQuizResult($post_id = false) {
    global $fl, $sqlConnect;
    if (empty($post_id)) {
        return false;
    }
    $post_id = FL_Secure($post_id);
    $table   = T_ENTRIES;
    $sql     = "SELECT * FROM `$table` WHERE `post_id` = '{$post_id}' AND `entry_type` = 'result' ORDER BY `id` ASC";
    $query = mysqli_query($sqlConnect, $sql) or die(mysqli_error($sqlConnect));
    $data = array();
    if ($query && mysqli_num_rows($query) > 0) {
        while ($fetched_data = mysqli_fetch_assoc($query)) {
...
{
    global $fl, $db;
    if (empty($ref) || $fl['loggedin'] == false) {
        return false;
    }
    $ref = FL_Secure($ref);
    $user = $db->where('user_id',$fl['user']['user_id'])->where('paystack_ref',$ref)->getValue(T_USERS,"COUNT(*)");
    if ($user < 1) {
        return false;
    }
    $result = array();
        ...
        if($result){
          if($result['data']){
            if($result['data']['status'] == 'success'){
                $db->where('user_id',$fl['user']['user_id'])->where('paystack_ref',$ref)->update(T_USERS,array('paystack_ref' => ''));
                return true;
            }else{
...
function CheckRazorpayPayment($payment_id, $data)
{
    ...
    $key_id = $fl['config']['razorpay_key_id'];
    $key_secret = $fl['config']['razorpay_key_secret'];
...
function InsertBankTrnsfer($inserted_data) {
    global $fl, $sqlConnect;
...
{
    global $sqlConnect, $fl,$db;
    $post_id = FL_Secure($post_id);
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "https://api.agora.io/v1/apps/".$fl['config']['agora_app_id']."/cloud_recording/acquire");
    curl_setopt($ch, CURLOPT_HTTPHEADER, array('Authorization: Basic '.base64_encode($fl['config']['agora_customer_id'].":".$fl['config']['agora_customer_certificate']),'Content-Type: application/json;charset=utf-8'));
    ...
    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "https://api.agora.io/v1/apps/".$fl['config']['agora_app_id']."/cloud_recording/resourceid/".$resourceId."/mode/mix/start");
    curl_setopt($ch, CURLOPT_HTTPHEADER, array('Authorization: Basic '.base64_encode($fl['config']['agora_customer_id'].":".$fl['config']['agora_customer_certificate']),'Content-Type: application/json;charset=utf-8'));
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
    curl_setopt($ch, CURLOPT_POSTFIELDS,'{
    "cname":"'.$cname.'",
...
{
    global $sqlConnect, $fl,$db;
    if (empty($data) || $fl['config']['agora_live_video'] != 1 || empty($data['resourceId']) || empty($data['sid']) || empty($data['cname']) || empty($data['uid']) || empty($data['post_id'])) {
        return false;
    }
    $post_id = FL_Secure($data['post_id']);

    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, "https://api.agora.io/v1/apps/".$fl['config']['agora_app_id']."/cloud_recording/resourceid/".$data['resourceId']."/sid/".$data['sid']."/mode/mix/stop");
    curl_setopt($ch, CURLOPT_HTTPHEADER, array('Authorization: Basic '.base64_encode($fl['config']['agora_customer_id'].":".$fl['config']['agora_customer_certificate']),'Content-Type: application/json;charset=utf-8'));
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
    curl_setopt($ch, CURLOPT_POSTFIELDS,'{
      "cname": "'.$data['cname'].'",
    ...
    if (!empty($data) && !empty($data->serverResponse) && !empty($data->serverResponse->fileList)) {
        $db->where('post_id',$post_id)->update(T_ENTRIES,array('video_url' => $data->serverResponse->fileList));
    }
    return true;
}
...
function GetAdminInvitation() {
    ...
    $data  = array();
    $site  = $fl['config']['site_url'] . '/register?invite=';
    while ($fetched_data = mysqli_fetch_assoc($query)) {
...
function GetUsersByName($name = '', $limit = 25) {
    global $sqlConnect, $fl;
    if ($fl['loggedin'] == false || !$name) {
        return false;
    }
    $user        = $fl['user']['id'];
    $name        = FL_Secure($name);
    ...
    if (!empty($limit) && is_numeric($limit)) {
        $limit      = FL_Secure($limit);
        $limit_text = 'LIMIT ' . $limit;
    }
    $sql   = "SELECT `user_id` FROM " . T_USERS . " WHERE `user_id` <> {$user} AND `username`  LIKE '%$name%' $limit_text";
    $query = mysqli_query($sqlConnect, $sql);
    while ($fetched_data = mysqli_fetch_assoc($query)) {
        $data[] = FL_UserData($fetched_data['user_id']);
    }
    return $data;
}
...
function Fl_LangsNamesFromDB($lang = 'english') {
    global $sqlConnect, $fl;
    $data  = array();
...
function Fl_LangsFromDB($lang = 'english') {
    global $sqlConnect, $fl;
    $data  = array();
...
function Fl_GetLangDetails($lang_key = '') {
    global $sqlConnect, $wo;
    if (empty($lang_key)) {
        return false;
    }
    $lang_key = FL_Secure($lang_key);
    $data     = array();
