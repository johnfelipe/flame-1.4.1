...
foreach ($latest_news as $key => $fl['top_news_data']) {
    $fl['top_news_data']['news']['page_type'] = 'quiz';
...
$text = '';
if (!empty($_GET['c_id'])) {
	if (in_array($_GET['c_id'], array_keys($fl['list_categories']))) {
...
$latest_page_news = $fl['latest_page_lists'] = FL_FetchDataFromDB($fetch_latest_news_page_data_array);

$fl['content'] = FL_LoadPage('latest/quizzes', array('TOP_4' => $top_news_html, 'TOP_BIG' => $big_top_news_html));
$fl['page_url']            = $fl['config']['site_url'].'/latest-quizzes'.$text;
