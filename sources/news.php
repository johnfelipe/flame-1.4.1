...
foreach (FL_GetPostTags($fl['news']['tags']) as $key => $tag) {
    $tags .= '<a class="tags news" href="{{LINK tags/' . $tag . '}}" data-ajax="?link1=tags&tag='.$tag.'">' . $tag . '</a> ';
}
...
$page_context = array(
    ...
    'STORY_AJAX_LINK' => '?link1=news&id='.$fl['news']['slug'].'-'.$fl['news']['id'],
    'STORY_CATEGORY_LINK' => '?link1=latest-news&c_id='.$fl['news']['category'],
    'STORY_USER_LINK' => '?link1=profile&u='.$fl['news']['publisher']['username'],
...
$fl['page_url']            = $fl['config']['site_url'].'/news/'.$_GET['id'];
