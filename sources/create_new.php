...
else if($fl['config']['go_pro'] == 1 && !FL_IsAdmin() && !FL_IsPRO() && $fl['user']['posts'] >= $fl['config']['user_max_posts']){
...
switch ($include_type) {
    ...
    case 'quiz':
        ...
        break;
    case 'trivia':
        $entry_type = FL_LoadPage("create-new/entries/trivia", array(
           'ENTRY_TIME' => $time,
           'ENTRY_TIME_2' => $time+1,
           'QUESTION'   => FL_LoadPage("create-new/entries/trivia_question",array(
                'ENTRY_TIME' => time() . rand(1111, 9999),
            ))
        ));
        break;
    case 'flip':
        $entry_type = FL_LoadPage("create-new/entries/flip", array(
           'ENTRY_TIME' => $time,
           'ENTRY_TIME_2' => $time+1,
           'QUESTION'   => FL_LoadPage("create-new/entries/flip_question",array(
                'ENTRY_TIME' => time() . rand(1111, 9999),
            ))
        ));
        break;
...
$fl['page_url']            = $fl['config']['site_url'].'/create-new?type='.$type;