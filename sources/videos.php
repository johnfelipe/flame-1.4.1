...
$fl['videos']['is_still_live'] = false;
$fl['videos']['live_sub_users'] = 0;
if (!empty($fl['videos']['stream_name']) && !empty($fl['videos']['live_time']) && $fl['videos']['live_time'] >= (time() - 10) && $fl['videos']['live_ended'] == 0) {
    $fl['videos']['is_still_live'] = true;
    $fl['videos']['live_sub_users'] = $db->where('post_id',$fl['videos']['id'])->where('time',time()-6,'>=')->getValue(T_LIVE_SUB,'COUNT(*)');
}
...
if ($fl['is_post_owner'] == true) {
	$videos_admin_option = FL_LoadPage("story/admin-options");
...
foreach (FL_GetStoryComments(array('post_id' => $fl['videos']['id'],'page' => $fl['page'])) as $key => $fl['comment']) {
    ...
    if (!empty($fl['videos']['stream_name']) && $fl['comment']['time'] <= $fl['videos']['live_time']) {
        $video_time = GetVideoTime($fl['videos']['time'],$fl['comment']['time']);
        ...
        $current_time = FL_Time_Elapsed_String($fl['comment']['time']);
    }
...
$page_context  =  array(
    ...
    'STORY_THUMB' => FL_GetMedia($fl['videos']['image']),
    ...
    'STORY_AJAX_LINK' => '?link1=videos&id='.$fl['videos']['slug'].'-'.$fl['videos']['id'],
    'STORY_CATEGORY_LINK' => '?link1=latest-videos&c_id='.$fl['videos']['category'],
    'STORY_USER_LINK' => '?link1=profile&u='.$fl['videos']['publisher']['username'],
...
$fl['page_url']            = $fl['config']['site_url'].'/videos/'.$_GET['id'];
