...
foreach (FL_GetPostTags($fl['lists']['tags']) as $key => $tag) {
    $tags .= '<a class="tags lists" href="{{LINK tags/' . $tag . '}}" data-ajax="?link1=tags&tag='.$tag.'">' . $tag . '</a> ';
}
...
$page_context  = array(
    ...
    'STORY_THUMB' => FL_GetMedia($fl['lists']['image']),
    ...
    'STORY_AJAX_LINK' => '?link1=lists&id='.$fl['lists']['slug'].'-'.$fl['lists']['id'],
    'STORY_CATEGORY_LINK' => '?link1=latest-lists&c_id='.$fl['lists']['category'],
    'STORY_USER_LINK' => '?link1=profile&u='.$fl['lists']['publisher']['username'],
...
$fl['lists']['page_type'] = 'lists';
$fl['lists']['page_name'] = 'latest-lists';
$fl['content'] = FL_LoadPage("story/content",$page_context);
$fl['page_url']            = $fl['config']['site_url'].'/lists/'.$_GET['id'];
