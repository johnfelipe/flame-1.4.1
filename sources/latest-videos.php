...
foreach ($latest_news as $key => $fl['top_news_data']) {
    $fl['top_news_data']['news']['page_type'] = 'videos';
...
$text = '';
if (!empty($_GET['c_id'])) {
	if (in_array($_GET['c_id'], array_keys($fl['video_categories']))) {
...
$latest_page_videos = $fl['latest_page_videos'] = FL_FetchDataFromDB($fetch_latest_news_page_data_array);

$fl['content'] = FL_LoadPage('latest/videos', array('TOP_4' => $top_news_html, 'TOP_BIG' => $big_top_news_html));
$fl['page_url']            = $fl['config']['site_url'].'/latest-videos'.$text;
?>