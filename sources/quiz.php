...
foreach (FL_GetPostTags($fl['quiz']['tags']) as $key => $tag) {
    $tags .= '<a class="tags quiz" href="{{LINK tags/' . $tag . '}}" data-ajax="?link1=tags&tag='.$tag.'">' . $tag . '</a> ';
}
...
$page_context = array(
    ...
    'STORY_THUMB' => FL_GetMedia($fl['quiz']['image']),
    ...
    'STORY_AJAX_LINK' => '?link1=quiz&id='.$fl['quiz']['slug'].'-'.$fl['quiz']['id'],
    'STORY_CATEGORY_LINK' => '?link1=latest-quizzes&c_id='.$fl['quiz']['category'],
    'STORY_USER_LINK' => '?link1=profile&u='.$fl['quiz']['publisher']['username'],
...
$fl['page_url']            = $fl['config']['site_url'].'/quiz/'.$_GET['id'];
